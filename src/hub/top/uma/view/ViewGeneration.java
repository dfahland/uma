    ...
    for (int i = 0; i<trace.length; i++) {
      ...
      for (DNode b : runCut) {
        ...
        for (DNode e : b.post) {
          ...
          //System.out.print("'"+build.getSystem().uniqueNames[e.id]+"' = '"+trace[i]+"' ? ");
          
          if (   !build.getSystem().uniqueNames[e.id].equals(trace[i])
              && !build.getSystem().properLabels[e.id].equals(trace[i])) continue;
    ...
    for (int i = 0; i<trace.length; i++) {
      ...
      if (!enabledEvents.isEmpty()) {
        ...
        for (DNode e : build.getSystem().fireableEvents) {
          if (   build.getSystem().uniqueNames[e.id].equals(trace[i])
              || build.getSystem().properLabels[e.id].equals(trace[i]))
          {
    ...
    while (i < upperBound) {
      ...
      if (fireEvent.post != null) {
        ...
        if (fireEvent.isCutOff) {
          ...
          trace.addLast(build.getSystem().uniqueNames[cutOff.id]);
        } else {
          for (DNode b : fireEvent.post) {
            runCut.add(b);
          }
