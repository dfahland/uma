...
public class ViewGeneration2 {
  ...
  public boolean extendByTrace(String[] trace, Map<String, String> e2t) {
    ...
    System.out.println("available transitions "+toString(build.getSystem().uniqueNames));
    ...
    for (int i = 0; i<trace.length; i++) {
      ...
      for (DNode b : state.marking) {
        ...
        for (DNode e : b.post) {
          ...
          // the transition's unique name or label should match the event
          if (   !build.getSystem().uniqueNames[e.id].equals(eventName)
              && !build.getSystem().properLabels[e.id].equals(eventName)) continue;
      ...
      if (succState == null) {
        ...
        if (fireEvent == null) {
          ...
          for (DNode e : build.getSystem().fireableEvents) {
            if (   build.getSystem().uniqueNames[e.id].equals(eventName)
                || build.getSystem().properLabels[e.id].equals(eventName))
  ...
  public LinkedList<String> generateRandomTrace2(int upperBound) {
    ...
    for (int i = 0; i<upperBound; i++) {
      ...
      marking = getSuccessorMarking_equiv(marking, fireEvent);
      trace.addLast(build.getSystem().uniqueNames[fireEvent.id]);
    }
    return trace;
  }
