  ...
  public void test_regression_cutOff () {
    ...
    try {
      ...
      for (DNode e : build.getBranchingProcess().allEvents) {
        ...
        cutOffCount++;
        if (sys.properLabels[e.id].equals("u")) {
          short otherId = build.getCutOffEquivalentEvent().get(e).id;
          assertEquals("cut-off event u mapped to v", "v", sys.properLabels[otherId]);
        }
        if (sys.properLabels[e.id].equals("x")) {
          short otherId = build.getCutOffEquivalentEvent().get(e).id;
          assertEquals("cut-off event x mapped to x", "x", sys.properLabels[otherId]);
        }
      }
