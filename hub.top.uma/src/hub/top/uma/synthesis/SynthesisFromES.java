...
public class SynthesisFromES {
  ...
  class Pair {
    ...
    public Pair(Event f, BitSet s) { first = f; second = s; }
    public Event getFirst() { return first; }
    public BitSet getSecond() { return second; }
    public void setFirst(Event val) { first = val; }
    public void setSecond(BitSet val) { second = val; }
  ...
  public PetriNet synthesize(EventStructure es) {
      ...
      for (BitSet ce : CE) {
        ...
        if (isCondition) {
          ...
          for (int i=0; i<ce.length(); i++) {
            if (ce.get(i)) {
              String tgtTransitionLabel = properNames[E_index.get(i).id];
    ...
    for (Pair pair: B.keySet()) {
      ...
      for (Integer e2 = x.nextSetBit(0);  e2 >= 0; e2 = x.nextSetBit(e2 + 1)) {
        Transition trans = E.get(E_index.get(e2));
    ...
    try {
      PetriNetIO_Out.writeToFile(pnet, path+"_bp2", PetriNetIO_Out.FORMAT_DOT, 0);
  ...
  public static void main(String args[]) throws Throwable {
    ...
    SynthesisFromES synth = new SynthesisFromES(sys.properLabels, fileName_system_sysPath);
