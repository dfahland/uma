...
public abstract class DNodeSys {
  ...
  /**
   * The abstract representation uses numeric IDs instead of Strings to
   * distinguish system parts. This arrays maps the IDs back to their original
   * unique name. It has to be filled by suitable constructors in a subclass.
   * Use {@link #finalize_setProperNames()} to set its values consistently with
   * {@link #nameToID}.
   */
  public String   uniqueNames[];      // translate IDs to names
  
  /**
   * The abstract representation uses numeric IDs instead of Strings to
   * distinguish system parts. This arrays maps the IDs back to their label.
   * There can be multiple different nodes with the same label. This array has
   * to be filled by suitable constructors in a subclass. Use
   * {@link #finalize_setProperLabels()} to set its values consistently with
   * {@link #getOriginalNodeLabel(DNode)}.
   */
  public String   properLabels[];      // translate IDs to names
	
  /**
   * The reverse map from original unique names to numeric identifiers. It is
   * the developers responsibility to keep {@link #uniqueNames} and this field
   * consistent. Each name uniquely has to identify a node in the original
   * system, i.e., there must not be two nodes with the same name.
   */
   ...
   * precondition events cannot be fired by themselves.
	...
	public static class EventPreSet {
	  public DNode[] conds;
   ...
   * {@link Constructor} of {@link DNodeSys}.
    ...
    DNode.nameTranslationTable = this.uniqueNames;  // everything happens wrt. to this class
	}
	
	/**
   * Initialize {@link #properLabels} to the labels of the system transitions.
   * The function should be consistent with {@link #getOriginalNodeLabel(DNode)}.
   * Typically, this function should be invoked after all nodes of the
   * original system have been translated.
   */
	protected abstract void finalize_setProperLabels();
  ...
  protected void finalize_generateIndexes() {
	 ...
	 *   system that has the given globalId. Useful for retrieving the system
   ...
   */
  public abstract Object getOriginalNode(DNode d);

  /**
   * @param d
   * @return the label of the node that caused the definition of {@link DNode} 'd' in this system 
   */
  public abstract String getOriginalNodeLabel(DNode d);
  
   /**
   * @param d
   * @return the node that represents {@link N} 'n' in this system
   */
  public abstract DNode getResultNode(Object n);
    ...
    for (DNode e : fireableEvents) {
      ...
      while (!queue.isEmpty()) {
        ...
        String nodeLabel = "'"+uniqueNames[f.id]+"'"+antiLabel+" ("+f.id+")["+f.globalId+"]";
